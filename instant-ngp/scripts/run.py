...
def parse_args():
	...
	# Import added argument for objectify project
	parser.add_argument("--poses_from", default="colmap", choices=["charuco", "colmap"], help="Poses are generated from either 'colmap' or 'charuco'. This selection plays role on cropping planes around the object (default 'charuco')")
	parser.add_argument("--save_surface", action="store_true", help="Indicator if surface is to be saved.")
...
def set_crop_size(testbed, crop_size):
	...
	min_diag = np.array([0, 0.5, 0])
	max_diag = np.array([0, 0, 0])
	bbox = ngp.BoundingBox(cen - min_diag, cen + max_diag * 0.5)

	#print(bbox.min)
	#print(bbox.max)
	print("Cropping the scene", flush = True)
	testbed.render_aabb.min =np.array([0.064,0.4,0.06])
	testbed.render_aabb.max =np.array([0.98,0.97,0.98])
...
if __name__ == "__main__":
	...
	#print(dir(testbed),flush=True)
 
	#testbed.m_mesh_render_mode = ngp.MeshRenderMode.VertexColors
	...
	if args.scene:
		...
		if(".json" in args.scene):
			with open(args.scene, 'r') as file:
				js = json.load(file)

			# Print the data
			print(js)
			poses_from = js["poses_from"]
			rembg = js["rembg"]
	...
	if args.save_mesh:
		...
		print("Here is threshhold that is set", args.marching_cubes_density_thresh, flush = True)
		thresh = args.marching_cubes_density_thresh if args.marching_cubes_density_thresh is not None else 2.5
		print(f"Generating mesh via marching cubes and saving to {args.save_mesh}. Resolution=[{res},{res},{res}], Density Threshold={thresh}", flush = True)
	
		if(poses_from == "charuco" and not rembg):
			print( " I am here", flush = True)
			set_crop_size(testbed = testbed, crop_size=0.5)
